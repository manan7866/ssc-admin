...
import { Lightbulb, Plus, Search, Edit2, Trash2, X, Save, RefreshCw, Upload, Download, CircleAlert as AlertCircle, CircleCheck as CheckCircle2, Loader as Loader2, FileText } from 'lucide-react';
...
const emptyForm = (): Partial<Theme> => ({
  name: '', slug: '', description: '', category: 'metaphysical', conceptualClusters: [], displayOrder: 0,
});
...
Spiritual Wisdom,spiritual-wisdom,"Wisdom teachings from Sufi masters",interdisciplinary,1
Mystical Poetry,mystical-poetry,"Poetry by Rumi, Hafiz, and other mystics",literature,2
Sacred Geometry,sacred-geometry,"Geometric patterns in Islamic art",art,3`;
...
export default function ThemesAdminPage() {
  ...
  const load = useCallback(async (pageNum: number) => {
    ...
    const res = await fetch(`/api/admin/cms/themes?${params}`);
  ...
  function openEdit(theme: Theme) { setEditing(theme); setForm({ ...theme }); setModalOpen(true); }
  ...
  async function handleSave() {
    ...
    const payload = {
      ...
      category: form.category || 'interdisciplinary',
      conceptual_clusters: form.conceptualClusters || [],
      display_order: form.displayOrder ?? 0,
    };
    await fetch('/api/admin/cms/themes', {
  ...
  function downloadTemplate() {
    ...
    a.href = url; a.download = 'themes_bulk_template.csv'; a.click();
  ...
  function parseCSV(text: string): BulkRow[] {
    ...
    return lines.slice(1).map((line) => {
      ...
      return {
        ...
        description: row.description || '', category: row.category || 'interdisciplinary',
    ...
    for (let i = 0; i < updated.length; i++) {
      ...
      try {
        const payload = {
          ...
          category: row.category || 'interdisciplinary',
          display_order: row.display_order ? Number(row.display_order) : 0,
        };
        const res = await fetch('/api/admin/cms/themes', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-semibold text-[#F5F3EE] flex items-center gap-2">
            <Lightbulb size={20} className="text-[#C8A75E]" />
            Themes
          </h1>
          <p className="text-[#AAB0D6] text-sm mt-1">Manage thematic categories for content</p>
        </div>
        ...
        <div className="bg-[#0B0F2A] border border-white/10 rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-white/5">
              {themes.length === 0 && (
                <tr><td colSpan={4} className="px-4 py-10 text-center text-[#AAB0D6]">No themes found</td></tr>
              )}
              {themes.map(t => (
                <tr key={t.id} className="hover:bg-white/3 transition-colors">
                  <td className="px-4 py-3">
                    <div className="text-[#F5F3EE] font-medium">{t.name}</div>
                    {t.description && <div className="text-[#AAB0D6] text-xs mt-0.5 line-clamp-1">{t.description}</div>}
                  </td>
                  <td className="px-4 py-3 hidden md:table-cell text-[#AAB0D6]">
                    <span className="px-2 py-0.5 rounded-full text-xs bg-white/5 text-[#AAB0D6] capitalize">
                      {t.category}
                    </span>
                  </td>
                  <td className="px-4 py-3 hidden lg:table-cell text-[#AAB0D6] text-xs">{t.displayOrder}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => openEdit(t)} className="p-1.5 text-[#AAB0D6] hover:text-[#C8A75E] hover:bg-white/5 rounded-lg transition-colors">
                        <Edit2 size={13} />
                      </button>
                      <button onClick={() => handleDelete(t.id)} disabled={deleting === t.id} className="p-1.5 text-[#AAB0D6] hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-colors disabled:opacity-50">
                        <Trash2 size={13} />
                      </button>
                    </div>
                  </td>
          ...
          <div className="px-4 py-3 border-t border-white/10 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => { const newPage = page + 1; setPage(newPage); load(newPage); }} disabled={themes.length < PAGE_SIZE} className="px-3 py-1 text-xs text-[#AAB0D6] bg-white/5 rounded-lg disabled:opacity-30 hover:bg-white/10">Next</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#0B0F2A] border border-white/10 rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/10">
              <h2 className="text-sm font-semibold text-[#F5F3EE]">{editing ? 'Edit Theme' : 'Add Theme'}</h2>
            ...
            <div className="px-6 py-5 space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    value={form.category ?? 'metaphysical'}
                ...
                <div>
                  <label className="block text-xs text-[#AAB0D6] mb-1.5">Conceptual Clusters (comma-separated)</label>
                  <input
                    value={form.conceptualClusters?.join(', ') ?? ''}
                    onChange={e => setForm(f => ({ ...f, conceptualClusters: e.target.value.split(',').map(s => s.trim()).filter(Boolean) }))}
                    placeholder="e.g., theology, ontology, ethics"
                    className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-[#F5F3EE] focus:outline-none focus:border-[#C8A75E]/50"
                  />
                </div>
                <div className="sm:col-span-2">
      ...
      {bulkOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="bg-[#0B0F2A] border border-white/10 rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/10">
              <div>
                <h2 className="text-sm font-semibold text-[#F5F3EE]">Bulk Upload Themes</h2>
                <p className="text-xs text-[#AAB0D6]/50 mt-0.5">Upload a CSV file to add multiple themes at once</p>
              </div>
            ...
            <div className="px-6 py-5 space-y-5">
                      ...
                      <tbody className="divide-y divide-white/4">
                        {bulkRows.map((row, i) => (
                          <tr key={i} className={
                            ...
                            <td className="px-3 py-2 text-[#AAB0D6]/50 hidden sm:table-cell capitalize">{row.category || 'interdisciplinary'}</td>
              ...
              {bulkDone && (
                <div className="flex items-center gap-2 p-3 rounded-lg bg-emerald-500/8 border border-emerald-500/20 text-emerald-400 text-sm">
                  ...
                  Upload complete. {bulkRows.filter(r => r._status === 'success').length} themes added/updated.
                </div>
              )}
            </div>
            ...
            <div className="px-6 py-4 border-t border-white/10 flex justify-end gap-3">
              ...
              {!bulkDone && (
                ...
                >
                  ...
                  {bulkUploading ? 'Uploading...' : `Upload ${bulkRows.length} Themes`}
                </button>
              )}
            </div>
          </div>
